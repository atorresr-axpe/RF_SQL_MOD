...
join (
    ...
    from (
      select 
        ...
        *
      from `dbt-fundamentals-463313`.jaffle_shop.orders
    ) a
    ...
    left outer join `dbt-fundamentals-463313`.stripe.payments c
    on a.id = c.orderid
...
left outer join `dbt-fundamentals-463313`.stripe.payments payments
